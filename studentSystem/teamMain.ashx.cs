...
namespace studentSystem
{
    ...
    /// banMain 的摘要说明
    /// </summary>
    public class banMain : IHttpHandler, IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            //    SqlHelper.ExecuteNonQuery("delete from class_teacher where CLASS_ID=@CLASS_ID", new SqlParameter("@CLASS_ID", id));
            //    ShowList(context);
            //}
            if (context.Session["Admin"] != null)
            {
                ...
                {
                    SqlHelper.ExecuteNonQuery("delete from class where CLASS_ID=@CLASS_ID", new SqlParameter("@CLASS_ID", json));
                    //    ShowList(context);
        ...
        private void ShowList(HttpContext context)
        {
            ...
            dt = SqlHelper.ExecuteDataTable("select * from (select *,ROW_NUMBER() over( order by CLASS_ID asc) as num from class_teacher p) s where s.num>@Start and s.num<@End",
                new SqlParameter("@Start", (PageNumber - 1) * 9),
                new SqlParameter("@End", PageNumber * 10));
            int totalCount = (int)SqlHelper.ExecuteScalar("select count(*) from class_teacher");//一共有多少条数据
            ...
            {
                pageData[i] = new { Href = "teamMain.ashx?PageNumber=" + (i + 1), Title = i + 1 };
            }
            var Data = new { teams = dt.Rows, PageData = pageData, TotalCount = totalCount, PageNumber = PageNumber, PageCount = pageCount };


            //try
            //{
            //    dt = SqlHelper.ExecuteDataTable("select * from class_teacher");
            //}
            //catch (Exception e)
            //{

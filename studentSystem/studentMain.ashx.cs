...
namespace studentSystem
{
    ...
    public class studentMain : IHttpHandler, IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            //    SqlHelper.ExecuteNonQuery("delete from student where STUDENT_ID=@STUDENT_ID", new SqlParameter("@STUDENT_ID", id));
            //    ShowList(context);
            //}
            if (context.Session["Admin"] != null)
            {
                ...
                {
                    SqlHelper.ExecuteNonQuery("delete from student where STUDENT_ID=@STUDENT_ID", new SqlParameter("@STUDENT_ID", json));
                    ShowList(context);
        ...
        private void ShowList(HttpContext context)
        {
            ...
            dt = SqlHelper.ExecuteDataTable("select * from (select *,ROW_NUMBER() over( order by STUDENT_ID asc) as num from student_class p) s where s.num>@Start and s.num<@End",
                new SqlParameter("@Start", (PageNumber - 1) * 9),
                new SqlParameter("@End", PageNumber * 10));
            int totalCount = (int)SqlHelper.ExecuteScalar("select count(*) from student_class");//一共有多少条数据
            ...
            {
                pageData[i] = new { Href = "studentMain.ashx?PageNumber=" + (i + 1), Title = i + 1 };//pageData由要点击的链接和当前表示的页数组成
            }
            var Data = new { students = dt.Rows, PageData = pageData, TotalCount = totalCount, PageNumber = PageNumber, PageCount = pageCount };
